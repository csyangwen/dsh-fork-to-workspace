...
export async function importSession(ctx, zip, targetWorkspace) {
  ...
  // 2) 已存在检查：**不覆盖、不拒绝**——id 冲突时自动改用新 id 导入（复制
  //    语义），方便同一台机器上的备份/复制到新工作区场景；跨机器导入 id 无
  //    冲突时保持原始 id。
  ...
  const meta = {
    ...header,
    id: targetId,
    cwd: targetWorkspace === undefined ? header.cwd : targetWorkspace.path,
  }
  ...
  for (let at = 0; at < events.length; at += APPEND_BATCH) {
    try {
      await persistence.append(targetId, events.slice(at, at + APPEND_BATCH))
    } catch (error) {
  ...
  // 6) 挂载到目标工作区。关键：workspaceRegistry 的成员索引（header cwd →
  //    工作区 path）只在**进程启动时**构建，运行中直接写入持久化的新会话
  //    不会被索引，即使 header.cwd 正确也会显示在「未分组」。attachSession
  //    会实时读 header、校验 canonical cwd 并登记成员关系，之后会话才出现在
  //    目标工作区分组下（与官方 fork 的挂载路径一致）。
  if (targetWorkspace !== undefined && typeof targetWorkspace.attachSession === 'function') {
    try {
      await targetWorkspace.attachSession(targetId)
    } catch (error) {
      warnings.push(`挂载到工作区失败（会话将显示在「未分组」）：${error instanceof Error ? error.message : String(error)}`)
    }
  }

  // 返回值：sessionId 为实际落库的 id；发生复制时附 copiedFrom（原 id）。
  return {
    sessionId: targetId,
