 ...
 *     → { ok, source: { sessionId, title, workspaceId, presetId }, workspaces: [...] }
 *     对话框打开时一次性取「源会话信息 + 现有工作区列表 + 可选预设列表」。
 *     presets 字段（agentPresets.list() 全量，含 trust/name/broken/isDefault）
 *     供分叉对话框的「预设选择」下拉使用。
 *
 *   POST /dsh-fork-ws/fork   body: { sessionId, workspaceId?, atSeq?, agentPreset? }
 ...
 *     agentPreset 可选：指定分叉后子会话使用的 agent 预设（默认继承源会话的
 *     预设）。换预设时会把种子事件里的 agent-preset/selected 一并改写（或追加），
 *     保证会话打开时 agentPresets 服务解析到的就是所选预设——官方解析规则是
 *     「事件里最新一次 selected 优先于 header」，只改 header 不会生效。
 *
...
/**
 * prepare：读取源会话（存在性 + 标题 + 所属工作区）、全部现有工作区，以及
 * 可选的 agent 预设列表（供分叉对话框的预设下拉使用）。
 * @param ctx - 宿主上下文。
...
async function prepare(ctx, sessionId) {
  ...
  // 可选预设列表：读失败降级为空数组（对话框只隐藏预设下拉，不阻塞分叉）。
  // list() 每进程按目录重扫一次，返回 { id, trust, path, name?, description?, broken? }。
  let presets = []
  const agentPresets = ctx.get('agentPresets')
  if (agentPresets !== undefined) {
    try {
      const defaultId = agentPresets.defaultId
      presets = (await agentPresets.list()).map(preset => ({
        id: preset.id,
        trust: preset.trust,
  ...
  return {
    ...
    source: {
      ...
      // 源会话实际运行的预设（事件里最新一次 selected 优先，其次 header）。
      presetId: resolveSessionPreset(state.header, state.events) ?? null,
    },
    workspaces,
    presets,
  }
 ...
 * 可选 agentPreset 指定子会话预设）。
 * @param ctx - 宿主上下文。
 * @param body - { sessionId, workspaceId?, atSeq?, agentPreset? }。
...
async function forkToWorkspace(ctx, body) {
  ...
  // agentPreset：可选预设 id；null/缺省 = 继承源会话当前预设（向后兼容）。
  const agentPreset = body?.agentPreset == null ? undefined : String(body.agentPreset)
  if (agentPreset !== undefined && !/^[a-z0-9][a-z0-9-]*$/i.test(agentPreset)) {
    return { ok: false, error: '无效的预设 id', code: 'invalid-preset-id' }
  }
  try {
    const registry = ctx.get('workspaceRegistry')
    ...
    // 显式指定的预设必须先通过存在性/可用性校验（在 agentPresets.list() 里查）：
    // 找不到 → preset-not-found；broken（配置损坏）→ preset-broken。校验失败
    // 一律拒绝，避免子会话创建成功却在打开时因预设组合失败而报废。
    if (agentPreset !== undefined) {
      const presets = ctx.get('agentPresets')
      if (presets === undefined) {
        return { ok: false, error: '预设服务不可用', code: 'internal' }
      }
      const found = (await presets.list()).find(preset => preset.id === agentPreset)
      if (found === undefined) {
        return { ok: false, error: `预设 "${agentPreset}" 不存在`, code: 'preset-not-found' }
      }
      if (typeof found.broken === 'string') {
        return { ok: false, error: `预设 "${agentPreset}" 不可用：${found.broken}`, code: 'preset-broken' }
      }
    }
    // 解析目标工作区：
    ...
    const childId = await forkSession(ctx, state, targetWorkspace, atSeq, agentPreset)
 ...
 * @param atSeq - 可选分支锚点。
 * @param agentPreset - 可选预设 id；缺省 = 继承源会话当前预设。
 * @returns 子会话 id。
 */
async function forkSession(ctx, state, targetWorkspace, atSeq, agentPreset) {
  ...
  // preset：显式指定优先，否则继承源会话实际运行的 preset（事件里最新一次
  // 选择，其次 header）。
  const presetId = agentPreset ?? resolveSessionPreset(state.header, events)
  const presets = ctx.get('agentPresets')
  let setup
  if (presets !== undefined) {
  ...
  // 换预设时（显式指定且与源不同），种子里的 agent-preset/selected 事件必须
  // 同步：官方 dsh-agent-presets 打开会话时「事件里最新一次 selected 优先于
  // header」，只改 header.agentPreset 会被事件里的旧值覆盖。这里把种子中最后
  // 一条 selected 事件改写为新预设；种子中没有则追加一条（seq 顺延）。
  let seed = events.slice(0, cut)
  if (agentPreset !== undefined) {
    const selectedAt = seed.findLastIndex(event => event.type === 'agent-preset/selected')
    if (selectedAt !== -1) {
      // 改写必须产出新对象（浅拷贝 + data 拷贝），不能原地改：seed 是
      // events 的浅拷贝，事件对象本身与源会话共享引用，原地改会污染源会话。
      const original = seed[selectedAt]
      seed = [
        ...seed.slice(0, selectedAt),
        { ...original, data: { ...original.data, agentPreset } },
        ...seed.slice(selectedAt + 1),
      ]
    } else {
      seed = [
        ...seed,
  ...
  try {
    await agents.create({
      sessionId: childId,
      // 种子 = 截至边界的完整事件前缀（官方 fork 同款；换预设时已改写/追加
      // selected 事件，保证打开即用所选预设组合 agent）。
      seed,
      meta: {
